 ...
 *  - Se D > T_turno (il turno sfora):
 *       primo turno completo integrato (fattore medio = 1 + degrado_massimo/2),
 *       poi tutto il resto al degrado massimo (fattore = 1 + degrado_massimo).
 *
...
export function applicaDegradoOperatore(tempoSecondiPerPezzo, quantita, T) {
  ...
  let tempoTotaleConDegradoSec;

  if (durataLottoSec <= T_turno_sec) {
    const rapporto = durataLottoSec / T_turno_sec;
    const fattoreMedio = 1 + (degrado_massimo / 2) * rapporto;
    tempoTotaleConDegradoSec = durataLottoSec * fattoreMedio;
  } else {
    const primoTurnoSec = T_turno_sec * (1 + degrado_massimo / 2);
    const oreResidueSec = durataLottoSec - T_turno_sec;
    const oreResidueConDegradoSec = oreResidueSec * (1 + degrado_massimo);
    tempoTotaleConDegradoSec = primoTurnoSec + oreResidueConDegradoSec;
  }
