...
// Mappa interna materiale → gruppo del modello taglio. Tenuta locale
// (non in T.taglio.gruppi[].materiali) per due motivi:
//   1. lookup O(1) senza scorrere il blocco JSON ad ogni chiamata
//   2. il campo .materiali nel JSON resta documentativo (utile per
//      consultazione.html e ispezione manuale), mentre la risoluzione
//      runtime usa questa tabella.
// Se un materiale viene aggiunto a un gruppo: aggiornare ENTRAMBI
// (questa mappa e l'array .materiali in comune.json).
const GRUPPO_TAGLIO = {
  '42CD4': 'standard', 'B7': 'standard', 'B7M': 'standard',
  'L7': 'standard',    'B16': 'standard',
  'AISI 304': 'inox', 'AISI 316': 'inox',
  'B8 cl.2':  'inox', 'B8M cl.2': 'inox',
  'F51': 'f51', 'F53': 'f51',
  'Nitronic': 'superleghe', '718': 'superleghe',
  '660':      'superleghe', '625': 'superleghe',
};

/**
 * Tempo di taglio di uno spezzone di barra, in secondi.
 *
 * MODELLO
 * -------
 * Lineare nel diametro, calibrato per gruppo materiale:
 *     t_sec = m * dia + q
 * Calibrato su dati misurati forniti dal responsabile officina.
 * Validato statisticamente: R² > 0.96 per tutti i gruppi,
 * R² > 0.999 per F51 e superleghe. Range validato: dia da 6 a
 * 80 mm; fuori range il modello extrapola linearmente senza
 * errore (resta una stima ragionevole per i valori vicini al
 * bordo, ma per Ø molto fuori scala potrebbe perdere accuratezza).
 *
 * GRUPPI MATERIALE
 * ----------------
 *   - standard   (42CD4, B7, B7M, L7, B16)
 *   - inox       (AISI 304, AISI 316, B8 cl.2, B8M cl.2)
 *   - f51        (F51, F53)
 *   - superleghe (Nitronic, 718, 660, 625)
 *
 * I coefficienti m, q sono letti da T.taglio.gruppi[gruppo]
 * (autoritativo, modificabili senza toccare il codice). La
 * risoluzione materiale → gruppo usa la mappa interna
 * GRUPPO_TAGLIO (vedi sopra) — duplicazione consapevole con
 * T.taglio.gruppi[].materiali (che resta documentativa per
 * consultazione.html). Se aggiungi un materiale a un gruppo:
 * aggiornare ENTRAMBI.
 *
 * RISOLUZIONE CHIAVE
 * ------------------
 * Pattern standard del codice (analogo a tempoTornituraBase,
 * tempoSfacciatura, tempoFantina): se mat === 'altro' la chiave
 * viene presa da materiale_speciale, altrimenti coincide con mat.
 * Il sotto-valore 'altro' + materiale_speciale='0' (Generico)
 * lancia errore esplicito perché il modello richiede la
 * specifica del materiale per scegliere il gruppo F51 vs
 * superleghe.
 *
 * RITORNO
 * -------
 * Tempo in secondi (numero positivo). NESSUN minimo applicato,
 * NESSUN arrotondamento. La modulazione costo qta-dipendente e
 * il floor €10/lotto sono gestiti dal caller via modulaCostoTaglio
 * + taglioFin (pattern di tiranti_unificato.js).
 *
 * INVARIANTI DEL DOMINIO
 * ----------------------
 * Questa funzione modella il taglio "a monte" (sega/troncatrice).
 * Invariante del dominio: la fantina è l'unica lavorazione che NON
 * dipende dal taglio a monte, perché taglia direttamente in macchina
 * durante la tornitura. Coerentemente, tempoFantina non chiama
 * calcolaTempoTaglio.
 *
 * @param {number} dia diametro nominale di taglio (mm)
 * @param {string} mat materiale principale del pezzo
 * @param {string} materiale_speciale solo se mat === 'altro'; deve
 *   essere uno tra F51, F53, Nitronic, 718, 660, 625. Il valore '0'
 *   (Generico) non è ammesso per il taglio.
 * @param {object} T tabella comune.json (legge T.taglio.gruppi)
 * @returns {number} tempo di taglio in secondi
 */
export function calcolaTempoTaglio(dia, mat, materiale_speciale, T) {
  // Risoluzione chiave materiale — pattern standard del codice.
  const chiave = mat === 'altro' ? materiale_speciale : mat;

  // Caso 'altro' + Generico: il modello non sa scegliere tra F51 e
  // superleghe, errore esplicito (sulla falsariga di tempoFantina).
  if (mat === 'altro' && (chiave === '0' || !chiave)) {
    throw new Error(
      `calcolaTempoTaglio: materiale_speciale '0' (Generico) non valido ` +
      `per il taglio. Selezionare F51, Nitronic, 718, 660 o 625.`
    );
  }

  const gruppo = GRUPPO_TAGLIO[chiave];
  if (!gruppo) {
    const validi = Object.keys(GRUPPO_TAGLIO).join(', ');
    throw new Error(
      `calcolaTempoTaglio: materiale "${chiave}" non classificato. ` +
      `Materiali validi: ${validi}.`
    );
  }

  const p = T.taglio?.gruppi?.[gruppo];
  if (!p) {
    throw new Error(`calcolaTempoTaglio: T.taglio.gruppi.${gruppo} mancante in comune.json`);
  }
  return p.m * dia + p.q;
}
...
/**
 * Setup taglio dinamico, in secondi.
 *
 * MODELLO
 * -------
 * Il setup del taglio è dominato dal numero di BARRE che l'operatore
 * deve fisicamente caricare in macchina, non da una costante per lotto.
 * Una commessa che richiede 1 barra ha setup molto minore di una che ne
 * richiede 10, anche a parità di pezzi. La formula:
 *
 *   lungh_per_pezzo = lungh_geometrica_pezzo + sfrido_mm
 *   lungh_totale    = lungh_per_pezzo · qta
 *   num_barre       = ceil(lungh_totale / lungh_barra_mm)
 *   setup_grezzo    = tempo_per_barra_sec · num_barre
 *   setup           = clamp(setup_grezzo, setup_min_sec, setup_max_sec)
 *
 * Override sul diametro: se la barra di partenza ha Ø ≥ soglia_diametro_max
 * il setup va direttamente al massimo (manovra meccanica più lenta, peso
 * della barra, carro ponte). Questo bypassa il clamp inferiore.
 *
 * Il `ceil` su num_barre è intenzionale: anche solo 1 cm di lunghezza
 * residua oltre l'ultima barra intera richiede comunque di caricare
 * un'altra barra in macchina. Modellare come `round` (o frazione) sotto-
 * dimensionerebbe il setup nei lotti che "sfiorano" il limite barra.
 *
 * LUNGHEZZA GEOMETRICA PEZZO
 * --------------------------
 * `lungh_geometrica_pezzo` è la lunghezza dello spezzone NETTA, prima
 * dello sfrido di taglio. È responsabilità del chiamante calcolarla
 * coerentemente con la geometria del prodotto:
 *   - tiranti / prigionieri / tiranti_occhio: `lungh_pezzo`
 *   - viti stampate:                          `lungh + sviluppo_testa`
 *   - viti fresate:                           `lungh + h_testa`
 *   - dadi torniti/fresati:                   `altez`
 *   - dadi stampati:                          `sviluppo`
 * Lo sfrido (sfrido_mm da tabella) è aggiunto QUI, una volta sola e
 * uniforme per tutti i prodotti — non va sommato di nuovo a monte.
 *
 * RITORNO
 * -------
 * Tempo di setup taglio in secondi per il LOTTO (non per pezzo). Il
 * chiamante lo converte in costo via `setupCosto(setup_sec, co1, qta)`.
 * NON include il costo orario, NON include il minimo €/lotto.
 *
 * @param {number} lungh_geometrica_pezzo - Lunghezza netta dello spezzone (mm),
 *   prima dello sfrido. Vedi convenzione per prodotto sopra.
 * @param {number} qta - Quantità di pezzi del lotto.
 * @param {number} dia_disp - Diametro disponibile / della barra di partenza (mm).
 *   Usato solo per l'override "barra grande → setup max".
 * @param {object} T - Oggetto comune.json (legge T.setup_taglio).
 * @returns {number} Setup taglio in secondi per il lotto.
 */
export function calcolaSetupTaglio(lungh_geometrica_pezzo, qta, dia_disp, T) {
  const cfg = T.setup_taglio;
  const lungh_per_pezzo = lungh_geometrica_pezzo + cfg.sfrido_mm;
  const lungh_totale = lungh_per_pezzo * qta;
  const num_barre = Math.ceil(lungh_totale / cfg.lungh_barra_mm);
  const setup_grezzo = cfg.tempo_per_barra_sec * num_barre;
  const setup_clamped = Math.max(
    cfg.setup_min_sec,
    Math.min(setup_grezzo, cfg.setup_max_sec)
  );
  return (dia_disp >= cfg.soglia_diametro_max) ? cfg.setup_max_sec : setup_clamped;
}

// --- SMUSSO -------------------------------------------------

// Stessa lista di MAT_STANDARD_SBAV (più sotto, accanto a calcolaSbavatura)
// e MAT_STANDARD_FANTINA (più sotto, accanto a tempoFantina). Tenute distinte
// per evitare accoppiamenti tra logiche separate; un'unificazione è un
// refactor a parte.
const MAT_STANDARD_SMUSSO = ['42CD4', 'B16', 'B7', 'L7', 'B7M', 'A105'];

const NOMI_DISPLAY_SMUSSO = {
  copiatore: 'Copiatore',
  tela:      'Tela',
  ceriotti:  'Ceriotti',
  tornio:    'Tornio',
...
/**
 * Interpolazione lineare con clamp [0,1] su (x - x_min) / (x_max - x_min).
 * Riusata da calcolaSmusso e calcolaSbavatura (entrambe in questo file).
 */
export function interpola(x, x_min, x_max, y_min, y_max) {
...
/**
 * SMUSSO pre-rullatura su 3 macchine dedicate (copiatore / tela /
 * ceriotti) + tornio CN come fallback finale. Struttura simile a
 * calcolaSbavatura (sotto in questo stesso file) ma estesa con un
 * livello in più di selezione (la tela) e con interpolazione 2D
 * specifica per la tela. Pipeline:
 *   - filtra le macchine compatibili (Ø + lunghezza)
 *   - tempo_ciclo: 1D sul Ø per copiatore/ceriotti, 2D media (Ø+L)/2
 *     per la tela (range Ø stretto ma range L molto ampio, una sola
 *     dimensione predittiva sottostima il tempo sui pezzi lunghi)
 *   - moltiplicatore_materiale interpolato sul Ø (1D) per tutte le
 *     macchine dedicate, indipendentemente dal tipo di interpolazione
 *     del tempo base
 *   - selezione macchina a 3 livelli (vedi sotto)
 *   - tornio CN come fallback (tempo fisso, NESSUN moltiplicatore)
 *
 * SELEZIONE A 3 LIVELLI
 * ---------------------
 *   1. qta <= soglia_qta_tela (default 20):
 *        lotto piccolissimo → tela (setup breve, ciclo crescente con L)
 *        fallback: ceriotti → copiatore → tornio
 *   2. n_smussi >= soglia_qta_normale (default 100):
 *        lotto grande → copiatore (setup 30min ammortizzato)
 *        fallback: ceriotti → tornio
 *   3. altrimenti (lotto medio):
 *        ceriotti (setup 15min, no min lunghezza)
 *        fallback: copiatore → tornio
 *
 * NOTA SULLE SOGLIE — la soglia tela è su `qta` (pezzi fisici), le
 * altre soglie sono su `n_smussi` (= qta * smussi_per_pezzo). Per i
 * tiranti (2 smussi/pezzo) le soglie si "dimezzano" effettivamente
 * in termini di qta: qta=50 → n_smussi=100 → preferenza copiatore.
 *
 * NOTA SUI DIAMETRI — tutti i confronti Ø nel calcolaSmusso sono su
 * `dia_disp` (diametro fisico della barra), NON sul nominale UNI.
 * Motivo: i range `T.smusso.{copiatore,tela,ceriotti}.diametro_*`
 * sono espressi in diametri MEDI/FISICI (es. copiatore.diametro_max
 * = 44.5 corrisponde al medio di M48; tela.diametro_max = 30.60 al
 * medio di M33). Usare dian rifiuterebbe a torto i pezzi al limite
 * alto del range. Per i pezzi che arrivano allo smusso il callsite
 * garantisce no tornitura, quindi dia_disp ≈ dia_medio.
 *
 * Rispetto alla sbavatura cambiano:
 *   - macchine dedicate: 3 (copiatore/tela/ceriotti) vs 2
 *     (sbavatrice_normale/ceriotti)
 *   - selezione a 3 livelli vs 2 (la tela aggiunge il livello "piccolo")
 *   - variabile di soglia normale: n_smussi (qta scalata per
 *     smussi_per_pezzo) anziché qta diretta
 *   - costo orario discriminato internamente sul Ø (regola del vecchio
 *     smussoCosto: dia_disp < 56 → co1, altrimenti co2) e ritornato come
 *     campo co_applicato — la sbavatura riceve invece `co` già risolto
 *     dal caller come argomento.
 *
 * NOTA — mutua esclusività con tornitura: il "copiatore" qui è la
 * stessa macchina fisica usata per la tornitura. Il rischio di doppio
 * setup NON si presenta perché lo SMUSS dedicato si attiva SOLO quando
 * il pezzo NON viene tornito (callsite: viti.js
 * `Math.abs(dia_disp - medio) < 0.5`, tiranti `!ha_tornitura`). Se in
 * futuro una di quelle condizioni viene rilassata, ricomparirà il
 * rischio di addebitare due volte il setup copiatore — in tal caso va
 * aggiunto un dedupe a livello di calcolaViti / calcolaTiranti.
 *
 * Il caller calcola il costo finale come:
 *   tempo_per_pezzo  = info.tempo_ciclo_sec * smussi_per_pezzo
 *   costo_ciclo      = tempo_per_pezzo * info.co_applicato      (poi × qta a livello lotto)
 *   costo_setup      = setupCosto(info.setup_sec, co1, qta)     (NB: co1, non co_applicato)
 * E applica il floor €10/lotto sul costo_ciclo (fuori da qui).
 *
 * IMPORTANTE — co_applicato è il costo orario per il CICLO, discriminato Ø<56
 * (regola del vecchio smussoCosto). Il SETUP usa invece co1 fisso, coerente
 * col pattern del modulo (sbavatura, taglio, rullatura, ecc. — vedi viti.js
 * e tiranti_unificato.js). Concettualmente: ciclo = lavoro macchina (varia
 * con Ø), setup = lavoro operatore (non varia con Ø).
 *
 * @param {number} dia_disp Ø fisico della barra (≈ dia_medio per i pezzi
 *   che arrivano allo smusso, perché il callsite garantisce no tornitura).
 *   I range T.smusso.* sono in diametri fisici, NON nominali — vedi nota
 *   sui diametri sopra.
 * @param {number} lungh_pezzo lunghezza pezzo (mm)
 * @param {string} mat codice materiale
 * @param {number} qta quantità lotto
 * @param {number} smussi_per_pezzo 1 (viti) o 2 (tiranti)
 * @param {number} co1 costo orario base €/sec
 * @param {number} co2 costo orario maggiorato €/sec (Ø ≥ 56)
 * @param {object} T tabella comune.json
 * @returns {{ macchina, nome_display, tempo_ciclo_sec, setup_sec, co_applicato }}
 *   co_applicato = costo orario già discriminato internamente sul Ø,
 *   da usare nei caller per il costo finale (evita di duplicare la
 *   regola Ø<56 nei due moduli che chiamano calcolaSmusso).
 */
export function calcolaSmusso(dia_disp, lungh_pezzo, mat, qta, smussi_per_pezzo, co1, co2, T) {
  const cfg = T.smusso;
  const mults = cfg.moltiplicatori_materiale;

  // Costo orario discriminato sul Ø fisico (regola del vecchio smussoCosto).
  // La soglia 56 è coerente con un Ø fisico, non nominale.
  const co_applicato = dia_disp < 56 ? co1 : co2;

  // Soglia di selezione: n_smussi totali del lotto (un tirante richiede
  // 2 smussi per pezzo, una vite 1)
  const n_smussi = qta * smussi_per_pezzo;

  // Classificazione materiale. Ordine importante: 'altro' va intercettato
  // PRIMA del check su MAT_INOX, perché MAT_INOX include 'altro' come
  // marker di "materiale difficile".
  let categoria;
  if (MAT_STANDARD_SMUSSO.includes(mat)) categoria = 'standard';
  else if (mat === 'altro')              categoria = 'altro';
  else if (MAT_INOX.includes(mat))       categoria = 'inox';
  else                                   categoria = 'altro';

  const macchine = ['copiatore', 'tela', 'ceriotti'];
  const candidati = [];

  for (const nome of macchine) {
    const m = cfg[nome];
    // Compatibilità dimensionale: range Ø in T.smusso.* sono fisici,
    // quindi confronto su dia_disp (vedi nota sui diametri nel JSDoc).
    if (dia_disp < m.diametro_min || dia_disp > m.diametro_max) continue;
    if (m.lunghezza_min != null && lungh_pezzo < m.lunghezza_min) continue;
    if (m.lunghezza_max != null && lungh_pezzo > m.lunghezza_max) continue;

    // Tempo base: 1D sul Ø per copiatore/ceriotti (range Ø ampio, L meno
    // predittiva). Per la tela si usa una 2D media (Ø e L pesati uguale):
    // il range L della tela è molto ampio (20-1100mm) e impatta il tempo
    // di processo quanto il Ø, quindi limitarsi al Ø sottostimerebbe i
    // pezzi lunghi e sovrastimerebbe quelli corti.
    let tempo_base;
    if (nome === 'tela') {
      const fatt_dia   = interpola(dia_disp,    m.diametro_min,  m.diametro_max,  0, 1);
      const fatt_lungh = interpola(lungh_pezzo, m.lunghezza_min, m.lunghezza_max, 0, 1);
      const fattore    = (fatt_dia + fatt_lungh) / 2;
      tempo_base = m.tempo_ciclo_min_sec + fattore * (m.tempo_ciclo_max_sec - m.tempo_ciclo_min_sec);
    } else {
      tempo_base = interpola(dia_disp, m.diametro_min, m.diametro_max,
                              m.tempo_ciclo_min_sec, m.tempo_ciclo_max_sec);
    }
    const mult_mat = mults[categoria];
    const mult = interpola(dia_disp, m.diametro_min, m.diametro_max,
                           mult_mat.min, mult_mat.max);
    const tempo_ciclo = tempo_base * mult;

    candidati.push({
      macchina: nome,
      nome_display: NOMI_DISPLAY_SMUSSO[nome],
  ...
  // Selezione macchina a 3 livelli — vedi JSDoc per il rationale.
  // Soglia tela: su `qta` (pezzi fisici). Soglia normale: su `n_smussi`.
  const soglia       = cfg.soglia_qta_normale;
  const soglia_tela  = cfg.soglia_qta_tela;
  const copiatore_ok = candidati.find(c => c.macchina === 'copiatore');
  const tela_ok      = candidati.find(c => c.macchina === 'tela');
  const ceriotti_ok  = candidati.find(c => c.macchina === 'ceriotti');

  if (qta <= soglia_tela) {
    // Lotto piccolissimo → tela; fallback ceriotti → copiatore
    if (tela_ok)      return tela_ok;
    if (ceriotti_ok)  return ceriotti_ok;
    if (copiatore_ok) return copiatore_ok;
  } else if (n_smussi >= soglia) {
    // Lotto grande → copiatore; fallback ceriotti
    if (copiatore_ok) return copiatore_ok;
    if (ceriotti_ok)  return ceriotti_ok;
  } else {
    // Lotto medio → ceriotti; fallback copiatore
    if (ceriotti_ok)  return ceriotti_ok;
    if (copiatore_ok) return copiatore_ok;
  }

  // Fallback: tornio CN (tempo fisso, NESSUN moltiplicatore materiale)
  return {
    macchina: 'tornio',
    nome_display: NOMI_DISPLAY_SMUSSO.tornio,
...
// Stessa lista di MAT_STANDARD_SMUSSO (sopra) e MAT_STANDARD_FANTINA
// (sotto). Tenute distinte per evitare accoppiamenti tra logiche
// separate; un'unificazione è un refactor a parte.
const MAT_STANDARD_SBAV = ['42CD4', 'B16', 'B7', 'L7', 'B7M', 'A105'];
...
/**
 * SBAVATURA post-stampaggio testa viti su 3 macchine (sbavatrice_normale
 * / ceriotti / tornio fallback). Gemello strutturale di calcolaSmusso
 * (sopra in questo stesso file): stesso schema dati (T.sbavatura ~ T.smusso),
 * stesso pattern di interpolazione su Ø, stessa logica di selezione a
 * soglia. Differenze:
 *
 *   - prima macchina: sbavatrice_normale (vs copiatore nello smusso)
 *   - variabile di soglia: qta diretta (vs n_smussi = qta×smussi_per_pezzo
 *     nello smusso)
 *   - co_applicato: il caller decide co1/co2 e lo passa già risolto come
 *     argomento `co` (vs calcolaSmusso che ritorna co_applicato discriminato
 *     internamente sul Ø)
 *
 * Usata solo dalle viti stampate. NON applicata alle viti 5931 inox/altro
 * stampate (V7 ibrido), dove la sbavatura è già inclusa nell'intestazione
 * testa (ramo E1). Il caller (calcolaViti) gestisce questo skip.
 *
 * Il caller calcola il costo finale come:
 *   tempo_per_pezzo = info.tempo_ciclo_sec
 *   costo_ciclo     = tempo_per_pezzo * co  (× qta a livello lotto)
 *   costo_setup     = setupCosto(info.setup_sec, co1, qta)
 * E applica il floor €10/lotto sul costo_ciclo (fuori da qui).
 *
 * NOTA — `lungh` qui è la "lunghezza sottotesta" del pezzo (non lungh_pezzo
 * nudo): è ciò che entra fisicamente nelle sbavatrici, le cui soglie
 * lunghezza_min/max sono calibrate su questa dimensione. Nel modulo
 * chiamante (viti.js) la variabile si chiama proprio `lungh`.
 *
 * @param {number} dian Ø nominale (mm)
 * @param {number} lungh lunghezza sottotesta (mm)
 * @param {string} mat codice materiale
 * @param {number} qta quantità lotto
 * @param {number} co costo orario applicato (€/sec; il caller decide co1/co2)
 * @param {object} T tabella comune.json (legge T.sbavatura)
 * @returns {{ macchina, nome_display, tempo_ciclo_sec, setup_sec, costo_totale }}
 */
export function calcolaSbavatura(dian, lungh, mat, qta, co, T) {
  const cfg = T.sbavatura;
  const mults = cfg.moltiplicatori_materiale;
  ...
  let categoria;
  if (MAT_STANDARD_SBAV.includes(mat))  categoria = 'standard';
  else if (mat === 'altro')             categoria = 'altro';
  else if (MAT_INOX.includes(mat))      categoria = 'inox';
  else                                  categoria = 'altro';
 ...
 * @param {boolean} [finitura_aggiuntiva=false] - Quando true, aggiunge una
 *   passata di finitura in coda a D_target (stesso Vc, stesso avanzamento f).
 *   Usato sul gambo viti al CN, dove la finitura è sempre presente. Non si
 *   applica al copiatore (formula chiusa) né alla fantina (modello lineare),
 *   né alle sfacciature (C/D/E1) e alla tornitura laterale testa 5931 (E2).
 * @returns {number} Tempo tornitura in secondi (0 se D_grezzo <= D_target).
 */
export function tempoTornituraBase(D_grezzo, D_target, L, mat, materiale_speciale, T, finitura_aggiuntiva = false) {
  ...
  // Passata di finitura: una passata in più al diametro finito
  // (equivalente a num_passate + 1, regola operativa CN viti).
  if (finitura_aggiuntiva) {
    const n_rpm = (Vc * 1000) / (Math.PI * D_target);
    tempo_min += L / (f * n_rpm);
  }
  return tempo_min * 60;
}
