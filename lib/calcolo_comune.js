 ...
 *
 * Formula empirica: `((raggio + 5) / 100) · 60 + 3`.
 *  - il +5 mm è sovrapercorso utensile (entrata + uscita oltre il bordo);
 *  - il /100 è l'avanzamento implicito di 100 mm/min in G1 radiale;
 *  - il +3 s è tempo morto di approccio/distacco utensile.
 *
 * Non scala con materiale. Da verificare se per inox/superleghe serve
 * un fattore correttivo — oggi non c'è.
 *
 * @param {number} raggio_mm - Raggio da sfacciare (mm).
 * @returns {number} Tempo sfacciatura in secondi.
 */
export function tempoSfacciatura(raggio_mm) {
  return ((raggio_mm + 5) / 100) * 60 + 3;
}
