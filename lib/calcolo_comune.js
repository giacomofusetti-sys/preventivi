...
/**
 * SMUSSO pre-rullatura su 3 macchine (copiatore / ceriotti / tornio
 * fallback). Algoritmo struct-identico a calcolaSbavatura (sotto in
 * questo stesso file):
 *   - filtra le macchine compatibili (Ø + lunghezza)
 *   - tempo_ciclo interpolato sul Ø nel range di ogni macchina
 *   - moltiplicatore_materiale anch'esso interpolato sul Ø
 *   - selezione macchina via SOGLIA su n_smussi (= qta * smussi_per_pezzo)
 *   - tornio CN come fallback (tempo fisso, NESSUN moltiplicatore)
 *
 * Pattern di selezione IDENTICO alla sbavatura: lotto grande → prima
 * macchina dedicata (qui copiatore, lì sbavatrice_normale); lotto
 * piccolo → ceriotti; fallback reciproco se la prima scelta è fuori
 * range; altrimenti tornio.
 *
 * Rispetto alla sbavatura cambiano:
 *   - variabile di soglia: n_smussi (qta scalata per smussi_per_pezzo)
 *     anziché qta diretta
...
export function calcolaSmusso(dian, lungh_pezzo, mat, qta, smussi_per_pezzo, co1, co2, T) {
  ...
  const macchine = ['copiatore', 'ceriotti'];
  const candidati = [];
  ...
  // Selezione macchina: stesso pattern della sbavatura (sotto).
  // Lotto grande (n_smussi >= soglia) → copiatore (con ceriotti come
  // fallback se fuori range). Lotto piccolo → ceriotti (con copiatore
  // come fallback). Se nessuna è compatibile → tornio.
