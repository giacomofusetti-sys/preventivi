 ...
 *     smussoCosto: dian < 56 → co1, altrimenti co2) e ritornato come
 ...
 */
export function calcolaSmusso(dian, lungh_pezzo, mat, qta, smussi_per_pezzo, co1, co2, T) {
  const cfg = T.smusso;
  const mults = cfg.moltiplicatori_materiale;

  // Costo orario discriminato sul Ø (regola del vecchio smussoCosto)
  const co_applicato = dian < 56 ? co1 : co2;
  ...
  for (const nome of macchine) {
    ...
    let tempo_base;
    if (nome === 'tela') {
      const fatt_dia   = interpola(dian,        m.diametro_min,  m.diametro_max,  0, 1);
      ...
      tempo_base = interpola(dian, m.diametro_min, m.diametro_max,
